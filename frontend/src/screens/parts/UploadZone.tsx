...
function validate(files: File[]): { ok: File[]; error?: string } {
  if (files.length > MAX_FILES)
    return { ok: [], error: `Up to ${MAX_FILES} files at a time.` };
  for (const f of files) {
    if (!SUPPORTED.includes(extOf(f.name)))
      return { ok: [], error: `Unsupported file: ${f.name}` };
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-ink-soft">
        Drop up to {MAX_FILES} audio files, or click to choose
      </p>
      <input
        ref={input}
        type="file"
