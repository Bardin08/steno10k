    ...
    <div className="grid grid-cols-[minmax(0,340px)_1fr] gap-6">
      <ul className="flex flex-col gap-2">
        {data?.map((a) => (
          <li
            key={a.name}
            className="flex items-center gap-2 rounded-sm border border-hairline bg-paper px-3 py-2.5"
          >
            ...
            <a
              href={downloadUrl(project, set, a.name)}
      ...
      <Card className="p-6">
        {selected ? (
          <Preview project={project} set={set} artifact={selected} />
        ) : (
          <p className="text-sm text-ink-soft">
            Select a text artifact to preview.
          </p>
        )}
      </Card>
    </div>
