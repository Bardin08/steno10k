...
is `small × 2`. See [Self-hosting](/steno10k/docs/self-hosting/#hardware--memory) before raising
either knob.

## Choosing a Whisper model

Only transcription is heavy — the LLM stages are remote calls. The model size is
your hardware floor, and each worker loads its own copy. The full per-model
table (disk, RAM per worker, quality, speed) lives in one place, alongside the
Docker memory guidance:
**[Self-hosting → Hardware & memory](/steno10k/docs/self-hosting/#hardware--memory)**.
